...
class NoteElement extends HTMLElement {
    constructor(data, notebookId) {
        ...
        <div class="card-body">
          ...
          <h6 class="card-subtitle mb-2 text-muted">${data ? new Date(data.date).toLocaleString() : new Date().toLocaleDateString()}</h6>
          <p class="card-text">${data ? data.message : 'invalid message'}</p>
